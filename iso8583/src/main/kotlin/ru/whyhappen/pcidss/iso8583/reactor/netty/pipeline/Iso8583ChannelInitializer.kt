...
import ru.whyhappen.pcidss.iso8583.reactor.netty.codec.StringLengthFieldBasedFrameDecoder
...
class Iso8583ChannelInitializer<T : ConnectorConfiguration>(
    ...
    /**
     * Handles exceptions thrown when decoding ISO messages.
     */
    ...
    override fun onChannelInit(
        ...
        with(channel.pipeline()) {
            ...
            if (configuration.replyOnError()) {
                addBefore(baseName, REPLY_ON_ERROR_HANDLER, decoderExceptionHandler)
            }
            if (configuration.shouldAddIdleEventHandler()) {
