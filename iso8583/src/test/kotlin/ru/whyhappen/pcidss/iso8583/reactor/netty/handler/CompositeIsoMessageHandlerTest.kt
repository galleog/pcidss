...
class CompositeIsoMessageHandlerTest {
    ...
    @MockK
    private lateinit var response: IsoMessage
    ...
    fun setUp() {
        ...
        every { message.mti } returns 0x200
    ...
    fun `should handle case when no handler found`() {
        val message2String = "IsoMessage(mti=0200)"

        every { messageHandler1.supports(message) } returns false
        every { messageHandler2.supports(message) } returns false
        every { message.toString() } returns message2String
        ...
        exceptionSlot.captured.message shouldBe "Message handler not found for $message2String"
    }
}